...
void InsertSort(Edge E[], int numof_e)
{ //此处应为插入排序
...
void Dijkstra(MatGraph g, int v)
{
    ...
    for (i = 0; i < g.n - 1; i++) {
        ...
        for (j = 0; j < g.n; j++) {
            if (visited[j] == 0) {
                if (g.edges[u][j] < INF && distance[u] + g.edges[u][j] < distance[j]) {
                    distance[j] = distance[u] + g.edges[u][j];
                    parent[j] = u;
                }
            }
        }
    }
    Dispath(g, distance, parent, visited, v);
}
