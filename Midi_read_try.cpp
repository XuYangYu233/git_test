...
long timeline=-1;       //时间轴
short yinyu_fixer=1;    //音域修正值，一般为2或1
...
void choose_dir_midi(char *path,char *midi_name){
    printf("Choose your music:\n");

    char mingling[128]="dir ";
    char namelist[50][128]={0};
    short sort=0;
    short respon;
    strcat(mingling,path);
    FILE *dir=NULL;
    dir = _popen(mingling, "r");
    while(!feof(dir)){
        fscanf(dir,"%s",mingling);
        if(strstr(mingling,".mid")!=NULL){
            strcpy(namelist[sort],mingling);
            printf("%hd. %s\n",++sort,mingling);
        }
    }
    fclose(dir);
    scanf("%hd",&respon);
    //respon = 31;
    //respon = (short)atoi(argv);
    if(respon==0){
        exit(0);
    }
    getchar();
    strcpy(midi_name,namelist[respon-1]);
    strcat(path,"\\");
    strcat(path,midi_name);
    return;
}
...
int Play_midi(){
    ...
    short num_of_loop=0;
    float us_per_tick;
    char path[128]="D:\\Study\\Python_work\\midi_try";   //
    char midi_name[128];

    choose_dir_midi(path,midi_name);
